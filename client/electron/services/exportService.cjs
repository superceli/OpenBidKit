...
/**
 * 调用 AI 生成绿色报告前置页内容（签章页描述）。扉页和目录由本地数据确定性生成。
 */
...
2. 只返回 JSON，不要任何解释文字或 Markdown 代码块`;
  ...
  let aiResult = {};
  try {
    aiResult = await aiService.requestJson({
      messages: [
    ...
    compactLogError('[export] AI 生成签章页声明失败，用默认', err);
  }
  ...
  return { titlePage, toc, signingPage };
}
