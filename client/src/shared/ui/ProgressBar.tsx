...
export default function ProgressBar({ value, label, tone = 'primary', active = false, showPercentage = false, className }: ProgressBarProps) {
  const clamped = Math.max(0, Math.min(100, Number.isFinite(value) ? value : 0));
  return (
    <span className={`yb-progress-wrapper${showPercentage ? ' has-percentage' : ''}`}>
      <span
      ...
      {showPercentage && (
        <span className="yb-progress-percentage">{Math.round(clamped)}%</span>
      )}
    </span>
  );
}
